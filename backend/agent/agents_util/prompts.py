...
DIAGNOSTIC_ASSISTANT_PROMPTS = {
    "diagnostic_intent": (
        ...
        "4. Classify the request into one of four categories: [DIAGNOSTIC]: if the user is asking to troubleshoot an issue, analyze the network, or providing past configurations to explain a current problem. [CONFIGURATION]: ONLY if the user is asking to apply NEW configurations or write commands, without an ongoing issue. [DIAGNOSTIC & CONFIGURATION]: if the user is investigating an issue AND explicitly asking to apply new configurations at the same time. [READ ONLY]: if the user is explicitly asking ONLY to view, read, or check the state of the network (e.g., 'show me routing tables', 'check interfaces') without reporting an issue to fix or asking for new configurations. [AGENT PROMPT]: if the user asks you to generate a prompt for another LLM agent to execute an experiment.\n"
        ...
        "8. If the category is [AGENT PROMPT], generate the detailed and comprehensive prompt text directly in the 'response' field, leave 'context' empty, and set status strictly to 'PROMPT_GENERATED'.\n"
        "--- STRICT RULES ---\n"
        ...
        "- NO MIXED PROMPT REQUESTS (CRITICAL): If the user asks to generate an [AGENT PROMPT] AND also asks for troubleshooting, reading, or applying configurations in the exact same message, you MUST set status to 'REJECTED' and explicitly ask the user to do not request for a prompt at the same time of other kind of requests.\n"
        ...
        "{\n"
        '  "status": "(string) \'APPROVED\', \'REJECTED\', or \'PROMPT_GENERATED\'",\n'
        '  "response": "(string) If REJECTED, write your clarifying questions or out-of-scope message. If PROMPT_GENERATED, write the generated prompt here. If APPROVED, leave empty.",\n'
        '  "context": "(string) Detailed summary of the issue to investigate. Leave empty string if REJECTED or PROMPT_GENERATED."\n'
        "}"
    ),
    "diagnostic_planner": (
        "--- ROLE ---\n"
